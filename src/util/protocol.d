...
*	}
*
*	mixin ProtocolPool!(int, VoidMessage, GendocArchive,
*		0, AMsg, 
*		1, BMsg,
*		2, CMsg
*		);
...
*
*	$(B PMessage) parameter should be class or interface to be inherited by all protocol messages. It is helpfull to use custom
*	opCall after getting deserialized message to pass additional info into (Tcp connection for instance).
*
*	Example:
*	--------
*	interface VoidMessage
...
*	-------- 
*/
mixin template ProtocolPool(IndexType, PMessage, SerializerBackend, pairs...)
{
	import std.stdio;
	import std.conv;
	...
	*	--------
	*/
	static assert(is(PMessage == interface) || is(PMessage == class), "PMessage ("~PMessage.stringof~") type must be interface or class!");
	/*interface PMessage
	{
		//string name;
		void opCall();
...
version(unittest)
{
	mixin ProtocolPool!(int, PMessage, GendocArchive,
	0, AMsg, 
	1, BMsg,
	2, CMsg
	);
